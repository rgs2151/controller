...
def _load_results(
    ...
    *,
    ...
    results = {}
    owner = benchmark or (
        "truthfulness" if behavior.startswith("truthfulness") else "toxicity"
    )
...
def _rows(
    ...
    capability_results: dict[tuple[str, str], dict],
) -> list[dict]:
    rows = []
    for model_key, model_label in MODELS:
        for method, markdown_label, tex_label in METHODS:
            ...
            capability = capability_results.get((model_key, method))
            if behavior == "toxicity":
                values = [
                    ...
                    _metric(capability, "mmlu_accuracy"),
                ]
            else:
                values = [
...
def render_markdown(toxicity_rows: list[dict], truthfulness_rows: list[dict]) -> str:
    ...
    lines.extend(
        [
            ...
            "MMLU is exact five-shot multiple-choice accuracy on one frozen "
            "1,000-question sample; its SE is the Bernoulli standard error across questions.",
            "",
        ]
    )
    lines.extend(
    ...
    lines.extend(
        [
            ...
            "repetitions. MMLU is exact five-shot multiple-choice accuracy on one "
            "frozen 1,000-question sample. Scorers run independently; TBD means that "
            "scorer has not been run.",
            "",
        ]
    )
    return "\n".join(lines)
...
def render_tex(toxicity_rows: list[dict], truthfulness_rows: list[dict]) -> str:
    lines = [
        ...
        r"\begin{table*}[!htbp]",
        r"\centering",
        r"\caption{Source-comparable toxicity benchmark with evaluated-model KV cache disabled. Toxicity, Dist-2, and PPL are mean $\pm$ SE across five complete 1,000-prompt repetitions for RTP and Jigsaw. MMLU is exact five-shot accuracy on one frozen 1,000-question sample, with Bernoulli SE across questions. TBD marks cells that have not been run.}",
        ...
        r"\toprule",
        r"Model & Method & RTP Tox. (\%) $\downarrow$ & RTP Dist-2 $\uparrow$ & RTP PPL $\downarrow$ & Jigsaw Tox. (\%) $\downarrow$ & Jigsaw Dist-2 $\uparrow$ & Jigsaw PPL $\downarrow$ & MMLU (\%) $\uparrow$ \\",
        r"\midrule",
        *_tex_rows(toxicity_rows),
        ...
        r"\begin{table*}[!htbp]",
        r"\centering",
        r"\caption{Truthfulness benchmark with evaluated-model KV cache disabled. True and Informative use the pinned TruthfulQA judges and rubrics; instruction relevance and fluency use the independent AXBench 0--2 rubrics. These values are mean $\pm$ SE across five complete 817-question repetitions. MMLU is exact five-shot accuracy on one frozen 1,000-question sample, with Bernoulli SE across questions. TBD marks scorers that have not been run.}",
        ...
        r"\toprule",
        r"Model & Method & ID True (\%) $\uparrow$ & ID Info. (\%) $\uparrow$ & ID Rel. (0--2) $\uparrow$ & ID Flu. (0--2) $\uparrow$ & Spanish True (\%) $\uparrow$ & Spanish Info. (\%) $\uparrow$ & Spanish Rel. (0--2) $\uparrow$ & Spanish Flu. (0--2) $\uparrow$ & MMLU (\%) $\uparrow$ \\",
        r"\midrule",
...
def main() -> None:
    ...
    toxicity_rows = _rows(
        "toxicity",
        _load_results("toxicity"),
        _load_results("toxicity_jigsaw"),
        _load_results("mmlu", benchmark="toxicity"),
    )
    truthfulness_rows = _rows(
        "truthfulness",
        _load_results("truthfulness"),
        _load_results("truthfulness_spanish"),
        _load_results("mmlu", benchmark="truthfulness"),
    )
