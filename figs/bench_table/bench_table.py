...
- Evaluation: the full protocol is 817 questions × 5 seeded repetitions. The promoted Llama-3-8B H∞ row and the GPT-2 XL and OLMo-2-32B blocks are 817-question single passes. The evaluated model uses KV cache off, temperature 1.0, top-p 0.3, and repetition penalty 1.2.
...
- Evaluation: the full protocol uses the same 817 prompt identities and repetition order as English TruthfulQA, repeated 5 times. The promoted Llama-3-8B H∞ row and the GPT-2 XL and OLMo-2-32B blocks are 817-question single passes. All rows use at most 50 new tokens, KV cache off, temperature 1.0, top-p 0.3, and repetition penalty 1.2.
- Example dataset item:
...
MGSM_MODELS = (
    ("qwen3_4b", "Qwen3-4B", ("original", "alqr", "h_infinity")),
    (
        "phi4_mini_instruct",
        "Phi-4-mini",
...
LCITE_LLAMA_METHODS = tuple(method for method in LCITE_METHODS if method[0] != "spid")
LCITE_METRICS = (
...
LCITE_LLAMA_JACKKNIFE_PATH = UNIT / "lciteeval/lciteeval_jackknife_llama32_1b.json"
LCITE_RESULT_OVERRIDES = {}
LCITE_MODELS = (
    {
        "key": "qwen25_3b_instruct",
        "label": "Qwen2.5-3B-Instruct",
        "tex_label": "Qwen2.5-3B",
        "tex_font": r"\scriptsize",
        "conditions": LCITE_CONDITIONS,
        "methods": LCITE_METHODS,
        "jackknife_path": LCITE_JACKKNIFE_PATH,
        "result_overrides": LCITE_RESULT_OVERRIDES,
    },
    {
...
- Dataset: `Jonaszky123/L-CiteEval`, pinned revision `c79c928529593f478e6573c969cf73d22f0cf0f9`, L-CiteEval-Length HotpotQA slice. The 40 question identities and gold answers are matched across both reported context lengths.
...
- Models: `Qwen/Qwen2.5-3B-Instruct` at revision `aa8e72537993ba99e69dfaafa59ed015b17504d1` and `meta-llama/Llama-3.2-1B-Instruct` at revision `9213176726f574b556790deb65791e0c5aa438b6`, both reported at 8K and 16K.
- The summary keeps the 8K and 16K context conditions separate.
...
Values are full-sample means ± ten-group delete-one-group jackknife standard errors. Each group contains four matched HotpotQA question identities; the same partition is used at 8K and 16K. The uncertainty measures question-sampling variability, not decoding-run or judge variability.
...
| Qwen2.5-3B-Instruct | H∞ | λ = 1.5; Q/R = 0.01; Qf/R = 0.01; R = 1; γ★ = 2.3054. |
...
def _rows(page: DatasetPage) -> list[dict]:
    rows = []
    models = TRUTHFULNESS_MODELS if page.benchmark == "truthfulness" else MODELS
...
def render_tex(page: DatasetPage, rows: list[dict]) -> str:
    ...
    uncertainty_note = (
        " Full-protocol rows report mean $\\pm$ standard error across five decoding repetitions; refreshed Llama-3-8B $H_\\infty$, GPT-2 XL, and OLMo-2-32B rows report five-group delete-one-group question-jackknife standard errors."
    ...
    lines = [
        ...
        f"\\caption{{{page.caption}{uncertainty_note}{incomplete_note}}}",
...
def _load_lcite_result(model: dict, condition: str, method: str) -> dict:
    root = RESULTS_ROOT / "lciteeval/results/kv_cache_off"
    calibration_id = model["result_overrides"].get((condition, method))
    if calibration_id is not None:
        root = root / "calibrations" / calibration_id
    path = root / model["key"] / f"hotpotqa_{condition}" / f"{method}.json"
    return json.loads(path.read_text())
...
def _lcite_jackknife(model: dict) -> dict:
    return json.loads(model["jackknife_path"].read_text())


def _lcite_rows(metrics: tuple[Metric, ...]) -> list[dict]:
    rows = []
    for model in LCITE_MODELS:
        jackknife = _lcite_jackknife(model)
        for condition_key, condition_label in model["conditions"]:
            for method_key, markdown_label, tex_label in model["methods"]:
                result = _load_lcite_result(model, condition_key, method_key)
                rows.append(
                    {
                        "model_key": model["key"],
                        "model": model["label"],
...
def render_lcite_markdown(rows: list[dict], *, full: bool) -> str:
    ...
    for row in rows:
        ...
        leading_values = f"{row['model']} | {row['condition']} | {row['method_markdown']}"
...
def render_lcite_tex(rows: list[dict], *, full: bool) -> str:
    ...
    caption = (
        "Full L-CiteEval length-transfer results. Values are full-sample means $\\pm$ ten-group matched-question jackknife standard errors. Context conditions use the same 40 question identities within each model."
        if full
        else "Summary L-CiteEval length-transfer results. Values are full-sample means $\\pm$ ten-group matched-question jackknife standard errors; available context conditions are reported separately within each model."
    )
    ...
    lines.append(r"\midrule")
    for model_index, model_spec in enumerate(LCITE_MODELS):
        model_rows = [row for row in rows if row["model_key"] == model_spec["key"]]
        for condition_index, (condition_key, condition_label) in enumerate(model_spec["conditions"]):
            group = [
            ...
            for method_index, row in enumerate(group):
                model = (
                    f"\\multirow{{{len(model_rows)}}}{{*}}{{\\rotatebox[origin=c]{{90}}{{{model_spec['tex_font']} {model_spec['tex_label']}}}}}"
                ...
                if method_index == 0:
                    lines.append(f"\\cmidrule(l){{3-{column_count + 3}}}")
            if condition_index != len(model_spec["conditions"]) - 1:
                lines.append(f"\\cmidrule(l){{2-{column_count + 3}}}")
        if model_index != len(LCITE_MODELS) - 1:
            lines.append(r"\midrule")
    lines.extend([r"\bottomrule", r"\end{tabular}%", r"}", r"\end{table*}", ""])
    return "\n".join(lines)
...
def render_false_reject_full_markdown(rows: list[dict]) -> str:
    lines = [
...
def render_false_reject_full_tex(rows: list[dict]) -> str:
    lines = [
        "% Generated by figs/bench_table/bench_table.py. Do not edit by hand.",
        r"\begin{table*}[!htbp]",
        r"\centering",
    ...
    for model_index, (model_key, model_label) in enumerate(HARMFUL_MODELS):
        ...
        if not model_rows:
            continue
        for condition_index, (condition_key, _, condition_short) in enumerate(
            HARMFUL_CONDITIONS
        ):
            condition_rows = [
                row for row in model_rows if row["condition_key"] == condition_key
            ...
            for method_index, row in enumerate(condition_rows):
                ...
                condition = (
                    f"\\multirow{{{len(condition_rows)}}}{{*}}{{{condition_short}}}"
                    if method_index == 0
                    else ""
                )
                formatted = []
...
def render_false_reject_summary_markdown(rows: list[dict]) -> str:
    lines = [
        "# HarmBench refusal-type analysis — summary",
        "",
    ...
    lines.extend(
        [
            "",
            "Direct and Adversarial regimes receive equal weight. USR is Direct Refusal + Safe Partial Compliance. Values are full-sample percentages ± ten-group behavior-clustered jackknife standard errors.",
            "",
        ]
    )
    return "\n".join(lines)
...
def render_false_reject_summary_tex(rows: list[dict]) -> str:
    lines = [
        "% Generated by figs/bench_table/bench_table.py. Do not edit by hand.",
        r"\begin{table*}[!htbp]",
        r"\centering",
        r"\definecolor{projectdarkred}{RGB}{128,0,0}",
        r"\caption{The FalseReject rubric applied to HarmBench outputs, summarized across conditions.}",
        r"\label{tab:harmbench-false-reject-summary}",
        r"\footnotesize",
    ...
    for model_index, (model_key, model_label) in enumerate(HARMFUL_MODELS):
        ...
        for method_index, row in enumerate(model_rows):
            ...
            for index, (value, standard_error) in enumerate(
                ...
                value_tex = f"${number}$"
                if index == 3:
                    value_tex = r"\cellcolor{projectdarkred!10}" + value_tex
                formatted.append(value_tex)
            lines.append(
...
def render_false_reject_reports() -> None:
    full_rows = _false_reject_full_rows()
    summary_rows = _false_reject_rows()
    if not full_rows or not summary_rows:
        return
    destination = UNIT / "harmful"
    destination.mkdir(parents=True, exist_ok=True)
    reports = (
        (
            "harmbench_false_reject_summary",
            render_false_reject_summary_markdown(summary_rows),
            render_false_reject_summary_tex(summary_rows),
        ),
        (
            "harmbench_false_reject_full",
            render_false_reject_full_markdown(full_rows),
            render_false_reject_full_tex(full_rows),
        ),
    )
    for stem, markdown, tex in reports:
        (destination / f"{stem}.md").write_text(markdown)
        (destination / f"{stem}.tex").write_text(tex)
        render_pdf(tex, destination / f"{stem}.pdf")
