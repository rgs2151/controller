...
def score(bundle_path: Path, device: str, *, cache_root: Path,
    ...
    fingerprint = {
        "input_sha256": sha256(bundle_path),
    ...
    if solution is not None:
        solution_digest = hashlib.sha256(
            solution.gains.detach().cpu().contiguous().numpy().tobytes()
        )
        solution_digest.update(
            json.dumps(
                json_safe(
                    {
    ...
    if destination.exists():
        ...
        if not manifest.exists() or read_json(manifest)["fingerprint"] != fingerprint:
            raise ValueError(
                "Run ID already exists with different or incomplete content; use a new run_id"
            )
...
def evaluate(path: Path, *, cache_root: Path) -> Path:
    ...
    if target.exists():
        if (target / "observations.json").read_bytes() != path.read_bytes():
            raise ValueError(
                "Evaluation ID exists with different content; use a new evaluation_id"
            )
        return target
