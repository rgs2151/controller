"""H-infinity calibration for HarmBench robust refusal."""
...
def _settings(model_key: str, q: float, r: float, q_final: float) -> dict:
    model = MODELS[model_key]
    return {
        "behavior": BENCHMARK,
        "seed": 2151,
        "fit_prompts_per_class": artifacts.FIT_RECORDS_PER_CLASS,
        "disturbance_prompts": 50,
        "calibration_max_length": artifacts.CALIBRATION_MAX_LENGTH,
...
def fit_base(
    ...
    *,
    ...
    calibration_data = {
        ...
        "dataset": {
            "direction": "HarmBench classifier validation completions, human majority",
            "disturbance_and_tuning": "50 direct non-copyright validation behaviors",
            "evaluation": "240 held-out non-copyright test behaviors",
        },
    }
...
def generate_worker(
    ...
    for configuration in grid()[shard_index::shard_count]:
        ...
        steering_policy = build_policy(
            "hinf", artifact, kp=0.0, ki=0.0, kd=0.0, record=True
        )
        rows = []
        for start in range(0, len(records), batch_size):
            ...
            trace = steering_policy.recorder.finish()
            energy = trace["control"].double().square().sum(dim=(0, 2)).tolist()
            rows.extend(
                {
                    ...
                    "text": row["raw_request"],
                    "concept": SAFE_CONCEPT,
                    "completion": completion,
                    "generated_tokens": token_count,
                    "control_energy": float(control_energy),
                }
                for row, completion, token_count, control_energy in zip(
                    batch, completions, generated, energy, strict=True
                )
            )
        _write_json(
            destination,
...
def score_grid(
    ...
    *,
    ...
    if len(generations) != len(grid()):
        raise ValueError("HarmBench H-infinity grid generations are incomplete")
    token = load_access_token(artifacts.REPO_ROOT)
    score_generations(
        generations,
        grid_root,
...
def _freeze_selected_diagnostics(
    ...
    problem["state_costs"] = parameters["q"] * identity.unsqueeze(0).repeat(
        horizon, 1, 1
    )
    problem["control_costs"] = parameters["r"] * identity.unsqueeze(0).repeat(
        horizon, 1, 1
    )
...
def select(model_key: str, calibration_id: str) -> dict:
    ...
    for configuration in grid():
        ...
        energies = [
            ...
            for repetition in generation_payload["repetitions"]
            for row in repetition["rows"]
        ]
        means = {"mean_control_energy": float(np.mean(energies))}
        for scorer in ("harmbench_validation_success", *API_SCORERS, "axbench_overall"):
            payload = json.loads(
...
def calibrate(
    ...
    *,
    ...
    if (root / "selection.json").exists() and (root / "controller.pt").exists():
        ...
        if not (root / saved["diagnostic_bundle"]).exists():
            raise FileNotFoundError("Selected H-infinity calibration lacks diagnostics")
        if fixed_parameters is not None:
            selected = saved["selected"]
            saved_fixed = {
                "q_over_r": float(selected["q_over_r"]),
                "q_final_over_r": float(selected["q_final_over_r"]),
                "r": float(selected["parameters"]["r"]),
            }
            if saved_fixed != fixed_parameters:
                raise ValueError(
    ...
    jobs = [
        (
            ...
            [
                sys.executable,
                "-m",
                "robust_steerability.benchmarks.harmful_calibration",
                "--stage",
                "generate-worker",
                "--model",
                model_key,
