"""H-infinity calibration for the ten-question L-CiteEval Small benchmark."""
...
from robust_steerability.benchmarks import lciteeval_small_artifacts as artifacts
from robust_steerability.benchmarks import lciteeval_small_runtime as runtime
...
GRID_SELECTION_SOURCE = "8K task-preservation and steering weighted calibration argmax"
...
SELECTION_WEIGHTS = {
    ...
    "citation_f1": 0.40,
    "fluency": 0.10,
    "concept_relevance": 0.10,
}
SELECTION_METRIC = "weighted_task_steering_score"
    ...
    *,
    ...
    for generation in generations:
        ...
        concept_payload = json.loads(
            scorer_cache_path(grid_root, generation, "axbench_concept_relevance").read_text()
        )
        concept = {row["prompt_id"]: float(row["score"]) / 2.0 for row in concept_payload["rows"]}
        fluency_payload = json.loads(
            scorer_cache_path(grid_root, generation, "axbench_fluency").read_text()
        )
        fluency = {row["prompt_id"]: float(row["score"]) / 2.0 for row in fluency_payload["rows"]}
        rows = [
            {
                "prompt_id": prompt_id,
...
def select(model_key: str, calibration_id: str) -> dict:
    ...
    for configuration in _grid():
        ...
        payload = json.loads(
            scorer_cache_path(root / "grid", generation, SELECTION_METRIC).read_text()
        )
        means = {
            SELECTION_METRIC: float(np.mean([float(row["score"]) for row in payload["rows"]])),
            "answer_recall": float(np.mean([float(row["answer_recall"]) for row in payload["rows"]])),
            "citation_f1": float(np.mean([float(row["citation_f1"]) for row in payload["rows"]])),
            "fluency_normalized": float(np.mean([float(row["fluency_normalized"]) for row in payload["rows"]])),
            "concept_relevance_normalized": float(
                np.mean([float(row["concept_relevance_normalized"]) for row in payload["rows"]])
            ),
        }
