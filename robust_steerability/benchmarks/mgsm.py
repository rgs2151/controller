"""Portable MGSM benchmark: artifacts, calibration, generation, and scoring."""
...
from robust_steerability.benchmarks import mgsm_calibration as calibration
    ...
    for method in ordered_methods:
        for language in datasets:
            ...
            command = [
                sys.executable,
                "-m",
                "robust_steerability.benchmarks.mgsm_runtime",
                "--stage",
                "generate-shard",
                "--model",
                model_key,
            ...
            run_data_shards(
                f"generate-{language}-{method}",
                command,
                devices,
                log_root / "generation" / method / language,
            )
...
def score_stage(
    ...
    selected_by_dataset = {}
    for language in datasets:
        selected = requested_scorers(COMPOSITION.dataset(language), scorers)
        selected_by_dataset[language] = selected
        for method in methods:
            ...
            for scorer in ("mgsm_exact_match", "axbench_rule_spanish"):
                if scorer in selected:
                    runtime.score_deterministic(
                        model_key, generation, scorer, use_cache=use_cache
                    )
    api_scorers = sorted(
        {
            scorer
    ...
    if api_scorers:
        openai_scoring.score_generations(
            [generation for _language, _method, generation in generations],
            runtime.cache_root(model_key, use_cache),
            api_scorers,
            concurrency=api_concurrency,
            batch_size=api_batch_size,
        )
    for language, _method, generation in generations:
        if "mgsm_axbench_overall" in selected_by_dataset[language]:
            runtime.score_overall(model_key, generation, use_cache=use_cache)
    for language, method, _generation in generations:
        runtime.summarize(
            model_key,
            language,
            method,
            selected_by_dataset[language],
            use_cache=use_cache,
        )
...
def main() -> None:
    ...
    run_id = arguments.run_id or default_run_id("mgsm", arguments.model, arguments.stage)
    with tracked_stage(
        run_id=run_id,
        benchmark="mgsm",
