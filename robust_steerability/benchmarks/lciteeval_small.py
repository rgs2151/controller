"""Portable ten-question L-CiteEval Small benchmark."""
...
from robust_steerability.benchmarks import lciteeval_small_calibration as calibration
...
COMPOSITION = load_composition("lciteeval_small")
    ...
    for method in ordered_methods:
        for condition in datasets:
            ...
            if runtime.generation_complete(destination):
                continue
            shard_count = 2 if len(devices) >= 8 else 1
            shard_counts[(method, condition)] = shard_count
            group = []
            for shard_index in range(shard_count):
                group.append(
                    (
                        f"generate-{condition}-{method}-shard-{shard_index:02d}",
                        [
                            sys.executable,
                            "-m",
            ...
            grouped_jobs.append(group)
    jobs = [
        group[shard_index]
        for shard_index in range(max((len(group) for group in grouped_jobs), default=0))
        for group in grouped_jobs
        if shard_index < len(group)
    ]
    if jobs:
