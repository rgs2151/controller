"""Build the frozen Corrupting Words and BOS Mix prompt sets."""
...
OUTPUT = CACHE / "prompt_attacks.json"
SEED = 2151
...
def _corrupting_words(
    records: list[dict[str, object]],
) -> list[dict[str, object]]:
    suffix = _suffix("text_only_transfer")
    return [
        {
            **record,
            "prompt_id": "corrupting-words:" + str(record["prompt_id"]),
            "prompt": str(record["prompt"]) + suffix,
            "text": str(record["prompt"]) + suffix,
            "construction": (
                "frozen text-only suffix found by gradient search to maximize "
                "A-LQR overshoot on one Llama-3.2-1B prompt"
            ),
        }
...
def _bos_mix(records: list[dict[str, object]], tokenizer) -> list[dict[str, object]]:
    ...
    for record in records:
        ...
        output.append(
            {
                **record,
                "prompt_id": "bos-mix:" + str(record["prompt_id"]),
                "prompt": prompt,
                "text": prompt,
                ...
                "construction": (
                    "seeded 16/64 mix of Gemma BOS tokens distributed evenly "
                    "through the prompt"
                ),
            }
        )
...
def main() -> None:
    ...
    attacks = {
        "corrupting_words": _corrupting_words(records),
        "bos_mix": _bos_mix(records, tokenizer),
    }
    payload = {
        "identity": {
            "schema_version": 3,
            ...
            "bos_token_id": tokenizer.bos_token_id,
            "source": (
                "parking/ood_adversarial completed text-only and BOS-mix recipes"
            ),
