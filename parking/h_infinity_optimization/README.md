...
- Preserve the reduced fit/calibration states, residuals, PCA basis, whitening maps, semantic targets, references, and disturbance construction. Compare empirical residual covariance with `D Dᵀ` using relative Frobenius error.
...
- Current benchmark coordinates are `normalized`: reduced states are whitened with calibration covariance; control and semantic outputs use measured calibration standard deviations; both running state and control costs are depth-weighted. The target-preserving reduced basis and nominal ridge dynamics are fit-only. Protected readouts span the orthogonal complement of the target in the reduced representation; they are not independently trained behavioral probes.
- Runtime uses full-state error about the retained fixed contrastive reference, with reference-consistent feedforward plus controller feedback. Toxicity and truthfulness use separately fitted targets and disjoint behavior-specific calibration records.
